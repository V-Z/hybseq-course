\documentclass[compress,  xelatex, 11pt, xcolor=x11names, aspectratio=169,
	hyperref={
		bookmarks=true,
		unicode=true,
		colorlinks=true,
...
\begin{frame}{Resources before we start}
	\begin{itemize}
		...
		\item Download presentation from \url{https://trapa.cz/en/hybseq-course-2026}
...
\begin{frame}{Zingiberaceae test data set}
	\begin{itemize}
		...
		\item The probes used for sequencing were introduced in \href{https://journals.sagepub.com/doi/full/10.1177/1176934317742613}{Fér et Schmickl 2018} (reduced to 30 probes)
...
\begin{frame}[allowframebreaks]{Steps from sequencing files to species trees}
	\begin{enumerate}
		...
		\item Reconstruction of gene trees from all aligned contigs (e.g. using \href{https://iqtree.github.io/}{IQ-TREE}, or \href{https://github.com/stamatak/ExaML}{ExaML}, \href{https://nbisweden.github.io/MrBayes/}{MrBayes}, \href{https://github.com/stephaneguindon/phyml}{PhyML}, \href{https://github.com/amkozlov/raxml-ng}{RAxML-NG},~\ldots)
...
\begin{frame}[allowframebreaks]{List of software used during the course}
	\begin{itemize}
		...
		\item \href{https://iqtree.github.io/}{IQ-TREE} --- gene trees
		...
		\item \href{https://phylogenomics.rice.edu/html/phylonet.html}{PhyloNet} --- phylogenetic networks
...
\begin{frame}{Running R tasks on MetaCentrum}
	\begin{itemize}
		...
		\item In the \texttt{metacentrum.sh} script load R, e.g. \texttt{module add r/4.5.1-gcc-10.2.1-zmneq6c} and start there R script as usually \texttt{R CMD BATCH script.r}
	\end{itemize}
	\begin{enumerate}
...
\begin{frame}[fragile]{Test data directory structure II}
	\begin{bashcode}
        ...
        I   ├── Aframomum-alboviolaceum_S118_L001.dedup.tar.gz # Sample output
        I   ├── ... # More samples...
        I   ├── Zingiber-officinale_S242_L001.dedup # Sample output file
        I   ├── exons # Directory with exons
        I   ├── introns # Directory with introns
        I   ├── supercontigs # Directory with supercontigs
        I   ... # Plenty of statistics, etc...
        I   ... # TSV and PNG files and logs
...
\begin{frame}{Scripts and other resources to process the data}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Reference for HybPiper \texttt{curcuma\_hybpiper\_renamed\_concat.fasta} in \texttt{ref/}
		\end{itemize}
	\end{itemize}
...
\begin{frame}[fragile]{Getting HybSeq scripts}
    ...
    pwd # See full path to the hybseq scripts - note it for later
    ls -lha # See content...
    module add r/4.5.1-gcc-10.2.1-zmneq6c # Load R module
...
\begin{frame}[fragile]{Install needed R packages}
    ...
    install.packages(pkgs = c("ape", "codetools", "digest", "fastmatch",
	...
	\end{spluscode}
	\begin{itemize}
		\item Installation might differ in different R versions --- in any case, ensure that packages \texttt{ape}, \texttt{ips} and \texttt{scales} can load correctly
	\end{itemize}
\end{frame}
...
\begin{frame}{Scripts to trim and deduplicate FASTQ sequences}
	\begin{itemize}
		...
		\begin{itemize}
			\item Submits via \texttt{qsub} script \texttt{$\sim$/bin/hybseq\_1\_prep\_2\_run.sh} which trims, deduplicates and quality checks the FASTQ data
...
\begin{frame}{When the pre-processing job is done}
	\begin{itemize}
		...
		\item Output directory \texttt{job\_123456789.pbs-m1} will be in \texttt{DATADIR} (\texttt{0\_data}) --- go there and move directories \texttt{1\_trimmed}, \texttt{2\_dedup} and \texttt{3\_qual\_rep} and log file \texttt{hybseq\_prepare.log} to same level as \texttt{0\_data}, e.g. \texttt{mv 1\_trimmed 2\_dedup 3\_qual\_rep hybseq\_prepare.log ../../}
...
\begin{frame}[fragile]{Sorting data after running HybPiper for all samples}
    ...
    # Move all HybPiper output directories there
    mv *.dedup.tar.gz HybPiper.* hybseq_hybpiper.*.log ../../../2_seqs/
...
\begin{frame}[fragile]{HybPiper postprocessing --- retrieving of contigs and statistics}
	\begin{itemize}
		...
		\item All outputs will be in \texttt{job\_123456789.pbs-m1} directory in \texttt{DATADIR} (\texttt{2\_seqs})
	\end{itemize}
	\begin{bashcode}
    # Create in ~/hybseq_course_zingiberss/2_seqs/ new samples_list.txt
    find . -maxdepth 1 -name "*.dedup.tar.gz" | \
      sed 's/^\.\///;s/\.tar\.gz$//' | sort > samples_list.txt
...
\begin{frame}{Results of HybPiper post-processing}
	\begin{itemize}
		\item Go to directory with results \texttt{$\sim$/hybseq\_course\_zingibers/2\_seqs/job\_123456789.pbs-m1/} --- there are directories with exons, introns and supercontigs with contigs to align in further step; and statistics of the post-processing (see mainly \texttt{hybpiper\_stats.tsv})
		\item Check files \texttt{presence\_of\_samples\_in\_contigs\_*.tsv} to see how many genes were reconstructed for each sample
		\item Move everything to \texttt{2\_seqs} directory by \texttt{mv * ../} and go to the \texttt{2\_seqs} directory (\texttt{cd ../}) --- now it is possible to submit alignment jobs (next chapter)
	\end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Gene trees from all alignments}
	\begin{itemize}
		...
		\item Script \texttt{$\sim$/hybseq/bin/hybseq\_5\_gene\_trees\_3\_run.sh} computes gene tree for every input file with \href{https://iqtree.github.io/}{IQ-TREE 3} (see how easily it can be converted for another tree builder, e.g. \href{https://github.com/amkozlov/raxml-ng}{RAxML-NG})
...
\begin{frame}[fragile]{Install IQ-TREE 3}
	\begin{itemize}
		\item Newest standalone version of IQ-TREE 3 is not installed on MetaCentrum, so we can download it and set path to it in \texttt{hybseq\_5\_gene\_trees\_2\_qsub.sh} --- \alert{Edit it!}
		\item See \url{https://iqtree.github.io/} to copy download URL
	\end{itemize}
	\begin{bashcode}
    cd # Ensure to be at home directory
    # Download the archive
    wget https://github.com/iqtree/iqtree3/releases/download/v3.1.2/
      iqtree-3.1.2-Linux.tar.gz # NOTE that whole URL is on a single line
    # Unpack it
    tar xzvf iqtree-3.1.2-Linux.tar.gz
    # Move the binaries into ~/bin (NOTE create the dir if it doesn't exist)
    mv iqtree-3.1.2-Linux/bin/* ~/bin/
    # Note the path for hybseq_5_gene_trees_2_qsub.sh
    realpath ~/bin/iqtree3
	\end{bashcode}
\end{frame}
...
\begin{frame}{Other tasks with gene trees}{For interested students}
	\begin{exampleblock}{Gene trees tasks for advanced users}
		\begin{enumerate}
			\item See \href{https://iqtree.github.io/doc/Command-Reference}{IQ-TREE help}, check its parameters in \texttt{hybseq\_5\_gene\_trees\_3\_run.sh} and think about possible changes to fit better your needs.
			...
			\item Check requirements of \href{https://iqtree.github.io/}{IQ-TREE} and/or another tree builder and think if changing of number of CPU threads and memory would significantly speed up processing of individual files.
			\item Replace usage of \href{https://iqtree.github.io/}{IQ-TREE} in \texttt{hybseq\_5\_gene\_trees\_3\_run.sh} by another tree builder like e.g. \href{https://github.com/stamatak/ExaML}{ExaML}, \href{https://nbisweden.github.io/MrBayes/}{MrBayes}, \href{https://github.com/stephaneguindon/phyml}{PhyML} or \href{https://github.com/amkozlov/raxml-ng}{RAxML-NG}.
		\end{enumerate}
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{Quick visualization of gene trees II}
	...
	\end{itemize}
	\begin{bashcode}
    module add r/4.5.1-gcc-10.2.1-zmneq6c # Load R. Ensure 'ape' is installed.
    # IQ-TREE
    ...
    for L in *.aln.fasta.raxml.support; do echo "${L}"; R CMD BATCH --no-save \
      ...
      mltreeplot.r; done
    # Note > " < around arguments to pass to R
	\end{bashcode}
\end{frame}
...
\begin{frame}[fragile]{Install needed R packages}
	\begin{itemize}
		...
		\item Use your notebook or e.g. MetaCentrum \href{https://docs.metacentrum.cz/en/docs/graphical/ondemand}{OnDemand} RStudio or \href{https://docs.metacentrum.cz/en/docs/software/sw-list/jupyter}{Jupyter notebook}
    ...
    install.packages(pkgs=c("ape", "ade4", "distory", "gplots", "ggplot2",
      "phangorn", "phytools"), repos="https://cloud.r-project.org/",
...
\begin{frame}[fragile]{Quick visualization of species trees II}
	...
	\end{itemize}
	\begin{bashcode}
    module add r/4.1.3-gcc-10.2.1-6xt26dl # Load R. Packages: ape, scales
    for L in sp_*.nwk; do echo "${L}"; R CMD BATCH --no-save --no-restore \
      "--args ${L} ${L%.nwk}.png" treeplot.r; done
    # Note > " < around arguments to pass to R
	\end{bashcode}
\end{frame}
