...
\begin{frame}{Scripts to trim and deduplicate FASTQ sequences}
	\begin{itemize}
		...
		\end{itemize}
		\item Script \texttt{$\sim$/bin/hybseq\_1\_prep\_2\_run.sh}
		\begin{itemize}
...
\begin{frame}[fragile]{Submission of hybseq\_1\_prep\_1\_qsub.sh}
	\begin{itemize}
		...
		\item \alert{\texttt{~/bin/hybseq\_1\_prep\_1\_qsub.sh} must be edited before submission via \texttt{qsub}}
...
\begin{frame}[allowframebreaks]{Alignment of all contigs}
	\begin{itemize}
		...
		\item Script \texttt{$\sim$/bin/hybseq\_run\_4\_alignment\_3\_postprocess.sh} is short, can be runned on the front end, it will sort outputs into subdirectories for exons, intron and supercontigs, create statistics of alignments and lists of NJ gene trees
	\end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Gene trees from all alignments}
	\begin{itemize}
		...
		\item Script \texttt{$\sim$/bin/hybseq\_run\_5\_gene\_trees\_3\_postprocess.sh} is short, can be runned on the front end, it will sort outputs into subdirectories for exons, intron and supercontigs and create lists of maximum likelihood and consensus (after bootstrapping) gene trees
	\end{itemize}
\end{frame}
