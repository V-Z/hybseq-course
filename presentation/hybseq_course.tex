...
\begin{frame}{HybSeq and its data}
	\begin{itemize}
		...
		\item It requires sequencing probes, preferably group specific (can be design using pipelines like \href{https://github.com/V-Z/sondovac/wiki}{Sondovač})
...
\begin{frame}[fragile]{Trimm and deduplicate all FASTQ files}
	\begin{exampleblock}{Tasks to pre-process FASTQ data for HybPiper}
			\begin{enumerate}
		\item Inspect \texttt{$\sim$/hybseq/hybseq\_1\_prep.sh} and be sure to understand what the script does, including syntax used.
		\item Inspect \texttt{$\sim$/bin/hybseq\_run\_1\_prep.sh} and be sure to understand what the script does, including syntax used.
		\item Edit declaration of variables \texttt{WORKDIR} and \texttt{DATADIR} so that they point to correct location.
		\item Submit via \texttt{qsub} \texttt{$\sim$/bin/hybseq\_run\_1\_prep.sh} to process both libraries.
