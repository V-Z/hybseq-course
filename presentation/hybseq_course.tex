...
\begin{frame}[fragile]{}
    ...
    # PCoA
    trees.pcoa <- dudi.pco(d=trees.d, scannf=FALSE, nf=3, full=FALSE, tol=1e-05)
    trees.pcoa
    # Plot PCoA
    ...
    # Run kdetrees to detect outliers
    trees.kde <- kdetrees(trees=trees, k=0.36, distance="dissimilarity", topo.only=FALSE, greedy=TRUE)
    trees.kde # See text results
    plot(x=trees.kde) # See graphical results
    plot.multiPhylo(trees.kde[["outliers"]]) # See removed trees
    # Save removed trees
    ...
    tree.sp <- superTree(tree=trees.good, method="NNI", rooted=TRUE, trace=2, start=NULL, multicore=TRUE, mc.cores=6)
